...
class appen:
    ...
    def find_cheaters(self, column_type = 'standard'):
        ...
        self.unique_appen_data   = self.appen_data.drop_duplicates(subset = [code_name])
        if column_type == 'standard':
            self.cheater_appen_data  = self.appen_data.drop(self.unique_appen_data.index)
            ...
            daniels_headers = ['ID', 'worker_id', 'worker_code', 'ip_address', 'start_time', 'end_time', 'country', 'gender', 'age', 'read_instructions', 'license_age', 'primary_transport', 'avg_vehicle_time', 'avg_mileage', 'accidents']
            self.cheater_appen_data.columns = daniels_headers
            
        else:
            pass
        print('There are %s cheaters detected, giving %s unreliable results.' %(len(self.cheater_appen_data['worker_code'].unique()), len(self.cheater_appen_data['worker_code'])))
